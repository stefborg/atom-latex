...
const BOX_PATTERN2 = new RegExp('' +
  '^((?:Over|Under)full \\\\[vh]box \\([^)]*\\))' + // Message text
  ' detected at line (\\d+)$'          // Line range
)
...
export default class LogParser extends Parser {
  ...
  parse () {
    ...
    lines.forEach((line, index) => {
      ...
      match = line.match(BOX_PATTERN2)
      if (match) {
        result.messages.push({
          type: 'warning',
          text: match[1],
          filePath: sourcePaths[0],
          range: [[parseInt(match[2], 10) - 1, 0], [parseInt(match[2], 10) - 1, Number.MAX_SAFE_INTEGER]],
          logPath: this.filePath,
          logRange: logRange
        })
        return
